...
https://github.com/gerefi/gerefi_documentation/issues/498

Need help with https://github.com/gerefi/gerefi_documentation/issues/499
...
### Troubleshooting hardware with gerEFI console
...
### Troubleshooting synchronization gerEFI console

Type `enable trigger_details` in gerEFI console to enable verbose synchronization logging. Post from 'Messages" output on the forum.

"print sync details to console" in TS enables the same output but the output still goes only to gerEFI console.
...
TIME TO UPDATE DOCUMENTATION SEE https://github.com/gerefi/gerefi_documentation/issues/497

Channel #1 counter would be "debug i1" gauge and channel #2 is "debug i2". These values should go up if you are cranking and that confirms that the events are being registered by gerEFI firmware.

![Debug Gauges](Images/debug_gauges_counter.png)

## Trigger Simulation

gerEFI has a feature of trigger signal emulation on Trigger Simulator Pins. All channels of trigger input would be simulated on corresponding channels of Trigger Simulator.

At the moment gerEFI has no means for VVT/camInput simulation.

## Q & A

**Q: what is _globalTriggerAngleOffset_ configuration parameter?**

A: On engine sniffer tab in gerEFI console, there is a signal front with "0" next to it. That's the trigger synchronization event on the primary trigger line. The trigger synchronization always happens at one of the rise of fall of the primary trigger. globalTriggerAngleOffset is the angle distance between synchronization point and cylinder #1 top dead center. TDC#1 is the green line by the way.
...
A: You can only use "4-stroke with Cam sensor" if your composite trigger shape is known to gerEFI. If you are adding a cam to 60/2, gerEFI probably does not know this combination with all the angles precisely. The way to add sequential to a skipped-tooth crank wheel is via cam input mode, same as used for VVT (see also [VVT](VVT))
