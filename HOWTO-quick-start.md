...
1. [Download the gerEFI bundle](Download) for your hardware.

2. Extract the bundle and launch the gerEFI console. Hit the "Install Drivers" button to install the Virtual Com Port and DFU drivers. If necessary the bundle will also help you install java run-time.

   ![console](FAQ/images/gerEFI_console_start_screen.png)

3. Get friendly on the [gerEFI forums](https://gerefi.com/forum) Introduce yourself, start a build thread, and ask questions.

4. [Download and Install TunerStudio](HOWTO-create-tunerstudio-project#download-and-install)

5. Start a new project, save to a folder that you specify, and choose the gerefi.ini file from the bundle. Or, even simpler, use 'Detect' button and let TunerStudio download .ini from the internet!
...
Your board comes with gerEFI firmware installed on it, but it should be updated.
...
3. Connect TunerStudio to your ECU. You will need a USB cable to establish communication between your tuning laptop and the gerEFI unit. There will be a USB port on a PnP unit.
...
Unplug your stock ECU and plug in your gerEFI PnP. You probably don't want to close the lid just yet, so you can observe blinking lights etc.
...
* Throttle Position Sensor (TPS)
 ...
 The values may show some jitters. RPMs should be 0. Push your throttle pedal and see if you get a response. Congratulations- your gerEFI can "see" your car.
...
A: There is some variation between vehicles and components. Please use [gerEFI forums](https://gerefi.com/forum) for custom installation help!

Q: I've made some changes in TunerStudio & hit Burn. 'Need Burn' has appeared but the values are not there after gerEFI reset?

A: gerEFI does not write to internal flash memory while your engine is running. In order to really write the values into the permanent memory you would need to stop the engine (or disconnect your stimulator). Online tuning is not affected by this issue - you can still tune and the new values would be applied right away, they just would not be saved until you stop the engine.

Q: I've remapped some analog inputs and outputs but nothing has changed, looks like the old pins are still used for output?

A: For most of the pinout changes to be applied you need to reset gerEFI. Do not forget to stop the engine so that the new config is saved into internal memory!

## gerEFI Console

[gerEFI Console](Console) is a handy development and debugging tool. Try it out and learn what it is capable of.

## Tuner Studio

gerEFI uses TunerStudio to allow you to configure and tune your ECU.
